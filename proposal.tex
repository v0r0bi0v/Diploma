...
%%
    ...
    \end{definition}
    \vskip 0.1in\noindent
    Then Zubkov defines a ring of generic matrices over $p$-adic numbers $\mathbb{Z}_p$ (the formal way to do it is quite long, see \cite)
    ...
    We are going to prove slightly different version of the theorem 3.2.1 for the case $\mathrm{char}(\Delta)=4$ and perhaps we will be able to do the similar thing for $\mathrm{char}(\Delta)=2^l$.
    Then we hope that the same calculations will complete the proof.

    Also, we will discuss the case $\mathrm{char}(\Delta)=0$ and how it can be investigated with aforementioned approaches.
