...
\begin{proof}
    ...
    \begin{multline*}
        ...
        \exp{\left( \LQ/\LQcentralr \cap M_2(S_n)/M_2(S_r) \right)}= \exp{(\LQn \oplus \ldots \oplus \LQ_{r-1})}
    \end{multline*}\\
    ...
    Введем $\tau: x_{i,j}\mapsto x_{i,j} + x_{i,j}^*, \tau: y_{i,j}\mapsto y_{i,j} + y_{i,j}^*$, продолжим его до автоморфизма $\overline{\psi}$ алгебры
    $M_2(\SQ)/M_2(\SQr)$.\\
    Заметим, что
    \[
