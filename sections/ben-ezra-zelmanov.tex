...
%Обозначим $\tilde{G}_n$ ~--- нижний центральный ряд группы $\tilde{G}$.
Введем отображения
\[
...
Рассмотрим подгруппу в $\tilde{G}^{(3)}\triangleleft\tilde{G}$ состоящую из элементов $1 + \sum a_i$, где $a_i \in (\trace(\tilde{X}))^3 J$.
И обозначим \[g_x = [(1 + \tilde{X})^4, g] = [1 + (\trace(\tilde{X}))^3 \tilde{X}, g]\]

Следующая лемма является ключевой

\begin{lemma}\label{lm:ben-ezra-zubkov-main}
...
\end{lemma}
\vskip 0.1in\noindent
Существование такого $h$ позволяет провести топологические рассуждения схожие с теми, что были у А.Н.\ Зубкова.

Но доказательство самой леммы~\ref{lm:ben-ezra-zubkov-main} весьма нетривиально.
